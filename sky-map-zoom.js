...
class NightSkyMapZoom {
    ...
    drawConstellations() {
        Object.entries(CONSTELLATIONS).forEach(([name, lines]) => {
            ...
            if (isSelected) {
                ...
                lineWidth = 1.5 * this.zoomLevel;
                this.ctx.strokeStyle = color;
                this.ctx.globalAlpha = 0.5;
            } else {
                // Faint constellation
                color = 'rgba(135, 206, 235, 0.2)';
                lineWidth = 1 * this.zoomLevel;
                this.ctx.strokeStyle = color;
            ...
            if (isSelected || (inView && inView.visibility > 0.6 && this.zoomLevel > 2.0)) {
                ...
                if (firstStar) {
                    ...
                    if (this.isInView(pos.x, pos.y)) {
                        ...
                        this.ctx.strokeText(name, pos.x, pos.y - 25 * this.zoomLevel);
                        this.ctx.fillText(name, pos.x, pos.y - 25 * this.zoomLevel);
                    }
                }
            }
        });
    ...
    drawStars() {
        this.allObjects.filter(obj => obj.type === 'star').forEach(star => {
            ...
            if (shouldShowName) {
                ...
                this.ctx.font = `${isInSelectedConstellation ? 'bold ' : ''}${Math.round(11 * this.zoomLevel)}px Arial`;
                ...
                if (isInSelectedConstellation) {
                    ...
                    this.ctx.fillRect(pos.x + size + 3, pos.y - 12, metrics.width + 4, 16);
    ...
    drawPlanets() {
        this.allObjects.filter(obj => obj.type === 'planet').forEach(planet => {
            ...
            this.ctx.fillStyle = '#FFD700';
            this.ctx.font = `bold ${Math.round(13 * this.zoomLevel)}px Arial`;
            this.ctx.textAlign = 'left';
            
            // Background for readability
            const metrics = this.ctx.measureText(planet.name);
            this.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
            this.ctx.fillRect(pos.x + size + 3, pos.y - 14, metrics.width + 4, 18);
    ...
    drawMoon() {
        ...
        this.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        this.ctx.fillRect(pos.x + radius + 3, pos.y - 14, metrics.width + 4, 18);
